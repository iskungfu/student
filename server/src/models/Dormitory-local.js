...
class Dormitory {
  static async findAll(page = 1, pageSize = 10) {
    ...
    const [rows] = await pool.execute('SELECT * FROM dormitories ORDER BY id DESC LIMIT ? OFFSET ?', [pageSize, offset]);
  ...
  static async update(id, dormitoryData) {
    ...
    values.push(id);
    await pool.run(`UPDATE dormitories SET ${updates.join(', ')} WHERE id = ?`, values);
    return await this.findById(id);
  }

  static async delete(id) {
    const result = await pool.run('DELETE FROM dormitories WHERE id = ?', [id]);
