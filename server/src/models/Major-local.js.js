...
class Major {
  static async findAll(page = 1, pageSize = 10) {
    ...
    const [rows] = await pool.execute('SELECT * FROM majors ORDER BY id DESC LIMIT ? OFFSET ?', [pageSize, offset]);
  ...
  static async update(id, majorData) {
    ...
    values.push(id);
    await pool.run(`UPDATE majors SET ${updates.join(', ')} WHERE id = ?`, values);
    return await this.findById(id);
  }

  static async delete(id) {
    const result = await pool.run('DELETE FROM majors WHERE id = ?', [id]);
