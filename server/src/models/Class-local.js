...
class Class {
  static async findAll(page = 1, pageSize = 10) {
    ...
    const [rows] = await pool.execute(
      `SELECT c.*, m.name as major_name FROM classes c
  ...
  static async update(id, classData) {
    ...
    values.push(id);
    await pool.run(`UPDATE classes SET ${updates.join(', ')} WHERE id = ?`, values);
    return await this.findById(id);
  }

  static async delete(id) {
    const result = await pool.run('DELETE FROM classes WHERE id = ?', [id]);
